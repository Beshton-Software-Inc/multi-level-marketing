...
class ReferralCode(Base):
    """Team-level referral codes generated by admins and distributed to affiliates.
    Each code is synced to the WWL backend on create/deactivate so WWL can route
    commissions at subscription time without calling back to MLM."""
    __tablename__ = "referral_codes"

    id                      = Column(Integer, primary_key=True, index=True)
    code                    = Column(String(20), unique=True, nullable=False, index=True)
    team_id                 = Column(Integer, ForeignKey("sales_teams.id"), nullable=False)
    created_by_affiliate_id = Column(Integer, ForeignKey("affiliates.id"), nullable=True)
    notes                   = Column(String, nullable=True)
    ...
    created_by       = relationship("Affiliate", foreign_keys=[created_by_affiliate_id])
